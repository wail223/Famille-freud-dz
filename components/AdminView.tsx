      ...
      <main className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 pb-[400px] custom-scroll">
        <section className="bg-slate-900 rounded-3xl p-5 border border-slate-800 shadow-2xl">
          <div className="flex flex-col gap-3">
            <input dir="rtl" type="text" placeholder="Ex: الماكلة في الدزاير..." className="w-full bg-black/50 border border-slate-700 rounded-2xl px-5 py-4 text-lg outline-none focus:border-amber-500 font-['Cairo']" value={themeInput} onChange={(e) => setThemeInput(e.target.value)} />
            <button disabled={loading || !themeInput} onClick={handleNewRound} className="w-full bg-amber-500 text-black font-black py-4 rounded-2xl text-xs uppercase tracking-widest active:scale-95 transition-transform">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {state.currentRound.top_10.map((ans, idx) => (
                <button key={ans.id} onClick={() => handleManualReveal(ans.id)} disabled={ans.revealed} className={`flex justify-between items-center p-5 rounded-2xl text-sm font-bold border transition-all active:scale-95 ${ans.revealed ? 'bg-green-600/10 text-green-400 border-green-600/30 opacity-50' : 'bg-slate-900 border-slate-800 shadow-lg'}`}>
      ...
      {state.currentRound && (
        <div className="fixed bottom-0 left-0 right-0 p-4 pb-[calc(20px+env(safe-area-inset-bottom))] z-50">
          <div className="bg-slate-900/95 backdrop-blur-2xl p-5 rounded-[2.5rem] border border-slate-700/50 shadow-[0_-20px_60px_rgba(0,0,0,0.8)] max-w-lg mx-auto">
            <input dir="rtl" type="text" value={guessInput} onChange={(e) => setGuessInput(e.target.value)} placeholder="Entrez la réponse..." className="w-full bg-transparent border-b-2 border-slate-700 py-3 mb-4 text-2xl text-center font-['Cairo'] outline-none focus:border-green-500" onKeyDown={(e) => e.key === 'Enter' && handleGuess()} />
